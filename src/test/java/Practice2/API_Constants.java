...
public class API_Constants extends FileReader{
    ...
    public static ResponseSpecification responseSpec(int status,long time,String key,String value){
        ResponseSpecification response= new ResponseSpecBuilder().expectStatusCode(status).expectResponseTime(lessThan(time)).expectBody(key,equalTo(value)).build();
        return response;
    }
}
